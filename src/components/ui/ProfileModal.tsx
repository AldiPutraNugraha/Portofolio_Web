...
export default function ProfileModal({ isOpen, onClose }: ProfileModalProps) {
  ...
  return (
    ...
    >
      ...
      {/* Modal Content */}
      <div
        className="relative bg-gradient-to-br from-slate-800 to-slate-900 rounded-3xl shadow-2xl max-w-2xl w-full border border-slate-700 overflow-hidden animate-scale-in"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Close Button */}
        <button
          onClick={onClose}
          className="absolute top-4 right-4 z-10 w-10 h-10 flex items-center justify-center rounded-full bg-slate-700/50 hover:bg-red-500 text-white transition-all duration-300 hover:rotate-90"
        >
          <svg
            className="w-6 h-6"
            fill="none"
        ...
        <div className="relative p-8">
          ...
          <div className="mt-8 flex gap-4 justify-center md:justify-end">
            <a
              href="#contact"
              onClick={onClose}
              className="px-6 py-3 bg-gradient-to-r from-purple-500 to-pink-500 text-white rounded-xl font-semibold hover:shadow-lg hover:shadow-purple-500/50 transition-all duration-300 hover:scale-105"
            >
              Hubungi Saya
            </a>
            <a
              href="#projects"
              onClick={onClose}
              className="px-6 py-3 border-2 border-purple-500 text-purple-400 rounded-xl font-semibold hover:bg-purple-500 hover:text-white transition-all duration-300 hover:scale-105"
            >
              Lihat Portfolio
            </a>
          </div>
